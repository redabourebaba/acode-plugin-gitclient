...
function logerror(msg){
  if(typeof window !== 'undefined'&& window.acode) {
    ...
    console.log(new Date(Date.now()).toISOString() + " | ISOGIT | ERROR | " + msg);
  }
}
...
function logwarn(msg){
  if(typeof window !== 'undefined'&& window.acode) {
    ...
    console.log(new Date(Date.now()).toISOString() + " | ISOGIT | WARN | " + msg);
  }
}
...
function loginfo(msg){
  if(typeof window !== 'undefined' && window.acode) {
    ...
    console.log(new Date(Date.now()).toISOString() + " | ISOGIT | INFO | " + msg);
  }
}
...
function logdebug(msg){
  if(typeof window !== 'undefined' && window.acode) {
    ...
    // console.log(new Date(Date.now()).toISOString() + " | ISOGIT | DEBUG | " + msg);
  }
}
...
async function updateCachedIndexFile(fs, filepath, cache) {
  ...
  loginfo('updateCachedIndexFile start');
  ...
  loginfo('updateCachedIndexFile end')
}
...
class GitIndexManager {
  ...
  static async acquire({ fs, gitdir, cache, allowUnmerged = true }, closure) {
    ...
    loginfo("GitIndexManager.acquire");
    ...
    await lock.acquire(filepath, async () => {
      try{
        ...
        logerror("GitIndexManager.acquire internal error. " + JSON.stringify(err));
      }
    }).catch(function(err) {
	    logerror("GitIndexManager.acquire error. " + JSON.stringify(err));
    });
...
function flatFileListToDirectoryStructure(files) {
  ...
  loginfo('flatFileListToDirectoryStructure start');
  ...
  loginfo('flatFileListToDirectoryStructure end');
...
class GitWalkerIndex {
  constructor({ fs, gitdir, cache }) {
    ...
    this.treePromise = GitIndexManager.acquire(
      ...
      async function(index) {
        loginfo('GitWalkerIndex.treePromise start');
        
        let result = flatFileListToDirectoryStructure(index.entries);
        
        loginfo('GitWalkerIndex.treePromise end');
        return result;
      }
    );
    const walker = this;
  ...
  async readdir(entry) {
    ...
    // logdebug('GitWalkerIndex.readdir start');
    ...
    if(tree !== undefined){
      ...
      if (inode.type !== 'tree') {
        let errorMsg = `ENOTDIR: not a directory, scandir '${filepath}'`;
        logerror('GitWalkerIndex.readdir error : ' + errorMsg);
        throw new Error(errorMsg);
      }
      const names = inode.children.map(inode => inode.fullpath);
      names.sort(compareStrings);
      
      // logdebug('GitWalkerIndex.readdir end');
  ...
  async stat(entry) {
    ...
    loginfo('GitWalkerIndex.stat start ' + entry._fullpath);
    
    if (entry._stat === false) {
      const tree = await this.treePromise;
      const inode = tree.get(entry._fullpath);
      if (!inode) {
        let errMsg = `ENOENT: no such file or directory, lstat'${entry._fullpath}'`;
        logerror('GitWalkerIndex.stat error ' + errMsg);
    ...
    loginfo('GitWalkerIndex.stat end ' + entry._fullpath);
  ...
  async oid(entry) {
    ...
    loginfo('GitWalkerIndex.oid start ' + entry._fullpath);
    ...
    loginfo('GitWalkerIndex.oid end ' + entry._fullpath);
...
class GitPackedRefs {
  constructor(text) {
    ...
    loginfo('GitPackedRefs.const start');
    ...
    loginfo('GitPackedRefs.const end');
  ...
  static from(text) {
    ...
    loginfo('GitPackedRefs.from start');
    
    let result = new GitPackedRefs(text);
    
    loginfo('GitPackedRefs.from end');
...
class GitRefManager {
  static async updateRemoteRefs({
    ...
    loginfo("GitRefManager.updateRemoteRefs");
  ...
  static async writeRef({ fs, gitdir, ref, value }) {
    ...
    loginfo("GitRefManager.writeRef ; " + ref);
  ...
  static async writeSymbolicRef({ fs, gitdir, ref, value }) {
    ...
    loginfo("GitRefManager.writeSymbolicRef");
  ...
  static async deleteRef({ fs, gitdir, ref }) {
    ...
    loginfo("GitRefManager.deleteRef");
  ...
  static async deleteRefs({ fs, gitdir, refs }) {
    ...
    loginfo("GitRefManager.deleteRefs");
  ...
  static async resolve({ fs, gitdir, ref, depth = undefined }) {
    ...
    loginfo("GitRefManager.resolve : ref=" + ref);
  ...
  static async expandAgainstMap({ ref, map }) {
    ...
    loginfo("GitRefManager.expandAgainstMap");
  ...
  static resolveAgainstMap({ ref, fullref = ref, depth = undefined, map }) {
    ...
    loginfo("GitRefManager.resolveAgainstMap");
  ...
  static async packedRefs({ fs, gitdir }) {
    ...
    loginfo("GitRefManager.packedRefs start");
    
    const text = await fs.read(`${gitdir}/packed-refs`, { encoding: 'utf8' });
    const packed = GitPackedRefs.from(text);
    
    loginfo("GitRefManager.packedRefs end : " + JSON.stringify(packed));
  ...
  static async listRefs({ fs, gitdir, filepath }) {
    ...
    loginfo("GitRefManager.listRefs");
    ...
    try {
      ...
      logerror('GitRefManager.list error ' + err);
      files = [];
    }
  ...
  static async listBranches({ fs, gitdir, remote }) {
    ...
    loginfo("GitRefManager.listBranches");
  ...
  static async listTags({ fs, gitdir }) {
    ...
    loginfo("GitRefManager.listTags");
...
class GitPackIndex {
  ...
  static async fromIdx({ idx, getExternalRefDelta }) {
    ...
    loginfo('GitPackIndex.fromIdx start');
    ...
    loginfo('GitPackIndex.fromIdx end');
...
async function loadPackIndex({
  ...
  loginfo('loadPackIndex start');
  
  const idx = await fs.read(filename);
  loginfo('loadPackIndex getExternalRefDelta=' + getExternalRefDelta);
...
async function readObjectPacked({
  ...
  loginfo('readObjectPacked start oid=' + oid);
  ...
  for (const filename of list) {
    ...
    loginfo('readObjectPacked reading ' + indexFile);
    ...
    if(p !== undefined){
      if (p.error) {
      logerror('readObjectPacked error ' + p.error);
      ...
      logerror('readObjectPacked error p is undefined');
    }
  }
  // Failed to find it
  return null
...
class GitWalkerRepo {
  ...
  async readdir(entry) {
    ...
    loginfo('GitWalkerRepo.readdir start '+ entry._fullpath);
    ...
    loginfo('GitWalkerRepo.readdir end ' + entry._fullpath);
...
class GitWalkerFs {
  ...
  async readdir(entry) {
    ...
    // logdebug('GitWalkerFs.readdir start');
    ...
    let result = names.map(name => join(filepath, name));
    
    // logdebug('GitWalkerFs.readdir end');
  ...
  async stat(entry) {
    ...
    loginfo('GitWalkerFs.stat start ' + entry._fullpath);
    ...
    if (entry._stat === false) {
      ...
      if (!stat) {
        ...
        logerror('GitWalkerFs.stat error ' + entry._fullpath);
    ...
    loginfo('GitWalkerFs.stat end ' + entry._fullpath);
  ...
  async oid(entry) {
    ...
    loginfo('GitWalkerFs.oid start ' + entry._fullpath);
    ...
    if (entry._oid === false) {
      ...
      await GitIndexManager.acquire({ fs, gitdir, cache }, async function(
        ...
        loginfo('GitWalkerFs.oid.aquire start ' + entry._fullpath);
        ...
        loginfo('GitWalkerFs.oid.aquire end ' + entry._fullpath);
    
      });
      entry._oid = oid;
    }
    
    loginfo('GitWalkerFs.oid end ' + entry._fullpath);
...
class FileSystem {
  ...
  async exists(filepath, options = {}) {
    try {
      ...
      let result = await this._stat(filepath);
  ...
  async read(filepath, options = {}) {
    try {
      loginfo("FileSystem.read start " + filepath);
      ...
      loginfo("FileSystem.read end " + filepath);
      
      return buffer
    } catch (err) {
      // logwarn("FileSystem.read error : " + err);
      return null
    }
  }
  ...
  async write(filepath, contents, options = {}) {
    try {
      loginfo('FileSystem.write ' + filepath);
      
      await this._writeFile(filepath, contents, options);
      return
    } catch (err) {
      
      loginfo("FileSystem.write error : " + err);
    ...
    const files = await Promise.all(
      subdirs.map(async subdir => {
        ...
        try {
          ...
          return stats && stats.isDirectory() ? this.readdirDeep(res) : res;
        } catch(err){
          return res;
        }
      })
...
class GitIgnoreManager {
  static async isIgnored({ fs, dir, gitdir = join(dir, '.git'), filepath }) {
    ...
    loginfo('GitIgnoreManager.isIgnored start ' + filepath);
    
    // ALWAYS ignore ".git" folders.
    if (basename(filepath) === '.git') {
      loginfo('GitIgnoreManager.isIgnored end always ignored ' + filepath);
      return true
    }
    
    // '.' is not a valid gitignore entry, so '.' is never ignored
    if (filepath === '.') {
      loginfo('GitIgnoreManager.isIgnored end never ignored ' + filepath);
      return false
    }
    ...
    for (const p of pairs) {
      ...
      if (parentdir !== '.' && ign.ignores(parentdir)) {
        loginfo('GitIgnoreManager.isIgnored end parent is ignored ' + filepath);
        return true
      }
    ...
    loginfo('GitIgnoreManager.isIgnored end ' + filepath);
...
async function _addRemote({ 
  ...
  loginfo("_addRemote save config");
...
async function parseRefsAdResponse(stream, { service }) {
  ...
  loginfo("parseRefsAdResponse start");
  ...
  loginfo('parseRefsAdResponse split');
...
class GitRemoteHTTP {
  ...
  static async discover({
    ...
    loginfo("GitRemoteHTTP.discover");
...
class GitShallowManager {
  static async read({ fs, gitdir }) {
    ...
    loginfo("GitShallowManager.read");
  ...
  static async write({ fs, gitdir, oids }) {
    ...
    loginfo("GitShallowManager.write");
...
async function _clone({
  ...
  try {
    ...
    loginfo("_init finished");

    await _addRemote({ fs, gitdir, remote, url, force: false });
    
    loginfo("_addRemote finished");
    ...
    loginfo("_checkout finished");
    
  } catch (err) {
    
    logerror("_clone error : " + err);
...
async function parseReceivePackResponse(packfile) {
  ...
  loginfo("parseReceivePackResponse start");
  ...
  try {
    ...
    return await _walk({
      ...
      map: async function(filepath, [head, workdir, stage]) {
        ...
        logdebug('statusMatrix._walk found start ' + filepath);
        ...
        if (!head && !stage && workdir) {
          if (!shouldIgnore) {
            ...
            if (isIgnored) {
              ...
              logdebug('statusMatrix._walk ignored ' + filepath);
        ...
        if (!filepaths.some(base => worthWalking(filepath, base))) {
          logdebug('statusMatrix._walk excluded ' + filepath);
        ...
        if (filter) {
          if (!filter(filepath)){
            ...
            logdebug('statusMatrix._walk filtered ' + filepath);
        ...
        logdebug('statusMatrix._walk found end ' + filepath);
 ...
 *
 * @example
 * await git.gittag({ fs, dir: '/tutorial', ref: 'test-tag' })
 * console.log('done')
 *
 */
...
export { Errors, STAGE, TREE, WORKDIR, abortMerge, add, addNote, addRemote,
annotatedTag, branch, checkout, clone, commit, currentBranch, deleteBranch,
deleteRef, deleteRemote, deleteTag, expandOid, expandRef, fastForward, fetch,
findMergeBase, findRoot, getConfig, getConfigAll, getRemoteInfo, getRemoteInfo2,
hashBlob, indexPack, init, isDescendent, isIgnored, listBranches, listFiles,
listNotes, listRemotes, listServerRefs, listTags, log, merge, packObjects, pull,
push, readBlob, readCommit, readNote, readObject, readTag, readTree, remove,
removeNote, renameBranch, resetIndex, resolveRef, setConfig, status,
statusMatrix, gittag, updateIndex, version, walk, writeBlob, writeCommit,
writeObject, writeRef, writeTag, writeTree };
